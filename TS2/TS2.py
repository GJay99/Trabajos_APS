...
sn_fft_power = 2 * (sn_fft_abs/N)**2
sn_fft_dB = 10 * np.log10(sn_fft_power)

freq, sn_cuantizada_fft_abs = mi_fft(sn_cuantizada)
sn_cuantizada_fft_power = 2 * (sn_cuantizada_fft_abs/N)**2
sn_cuantizada_fft_dB = 10 * np.log10(sn_cuantizada_fft_power)

freq, n_fft_abs = mi_fft(n)
n_fft_power = 2 * (n_fft_abs/N)**2
n_fft_dB = 10 * np.log10(n_fft_power)

n_q = sn_cuantizada - sn

freq, n_q_fft_abs = mi_fft(n_q)
n_q_fft_power = 2 * (n_q_fft_abs/N)**2
n_q_fft_dB = 10 * np.log10(n_q_fft_power)
plt.figure(2)
